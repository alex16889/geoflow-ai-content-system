...
class DatabaseAdmin {
    ...
    private function createTables() {
        ...
        CREATE TABLE IF NOT EXISTS keywords (
            ...
            cpc NUMERIC(12,4) DEFAULT NULL,
            competition VARCHAR(50) DEFAULT '',
            competition_index INTEGER DEFAULT NULL,
            monthly_searches_json TEXT DEFAULT '',
            metrics_updated_at TIMESTAMP DEFAULT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    private function insertDefaultData() {
        ...
        $bootstrapAdmin = build_initial_admin_credentials();

        $sql = "
        -- 插入默认管理员
        INSERT INTO admins (username, password, display_name, role, status, updated_at) VALUES (" . $this->pdo->quote($bootstrapAdmin['username']) . ", " . $this->pdo->quote($bootstrapAdmin['password_hash']) . ", '系统超级管理员', 'super_admin', 'active', CURRENT_TIMESTAMP);
        ...
        try {
            $this->pdo->exec($sql);
            store_bootstrap_admin_notice($bootstrapAdmin);
        } catch (PDOException $e) {
            // 忽略插入错误，可能是重复插入
        }
    }
    ...
    private function ensureCompatibilitySchema() {
        $columnsToAdd = [
            ...
            'keywords' => [
                ...
                'source' => "ALTER TABLE keywords ADD COLUMN source VARCHAR(50) DEFAULT 'manual'",
                'seed_keyword' => "ALTER TABLE keywords ADD COLUMN seed_keyword VARCHAR(200) DEFAULT ''",
                'location_code' => "ALTER TABLE keywords ADD COLUMN location_code INTEGER DEFAULT NULL",
                'language_code' => "ALTER TABLE keywords ADD COLUMN language_code VARCHAR(20) DEFAULT ''",
                'search_volume' => "ALTER TABLE keywords ADD COLUMN search_volume INTEGER DEFAULT NULL",
                'cpc' => "ALTER TABLE keywords ADD COLUMN cpc NUMERIC(12,4) DEFAULT NULL",
                'competition' => "ALTER TABLE keywords ADD COLUMN competition VARCHAR(50) DEFAULT ''",
                'competition_index' => "ALTER TABLE keywords ADD COLUMN competition_index INTEGER DEFAULT NULL",
                'monthly_searches_json' => "ALTER TABLE keywords ADD COLUMN monthly_searches_json TEXT DEFAULT ''",
                'metrics_updated_at' => "ALTER TABLE keywords ADD COLUMN metrics_updated_at TIMESTAMP DEFAULT NULL",
            ],
            'title_libraries' => [
    ...
    private function ensureGrowthAutomationSchema(): void {
        $articleColumns = [
            'quality_score' => "ALTER TABLE articles ADD COLUMN quality_score INTEGER DEFAULT NULL",
            'quality_issues' => "ALTER TABLE articles ADD COLUMN quality_issues TEXT DEFAULT ''",
            'quality_checked_at' => "ALTER TABLE articles ADD COLUMN quality_checked_at TIMESTAMP DEFAULT NULL",
        ];

        foreach ($articleColumns as $column => $sql) {
            if (!db_column_exists($this->pdo, 'articles', $column)) {
                $this->pdo->exec($sql);
            }
        }

        $this->pdo->exec("
            CREATE TABLE IF NOT EXISTS url_indexing_queue (
                id BIGSERIAL PRIMARY KEY,
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS idx_url_indexing_queue_site_provider_url
                ON url_indexing_queue(site_id, provider, url);
            CREATE INDEX IF NOT EXISTS idx_url_indexing_queue_status
                ON url_indexing_queue(provider, status, queued_at);

            CREATE TABLE IF NOT EXISTS site_api_spend (
                id BIGSERIAL PRIMARY KEY,
                site_id BIGINT NOT NULL REFERENCES sites(id) ON DELETE CASCADE,
                provider VARCHAR(40) NOT NULL,
                event_type VARCHAR(80) NOT NULL DEFAULT 'api_call',
            ...
            CREATE INDEX IF NOT EXISTS idx_site_api_spend_site_provider_day
                ON site_api_spend(site_id, provider, spent_at DESC);
        ");
    }
