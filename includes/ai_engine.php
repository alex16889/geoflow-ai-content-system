...
class AIEngine {
    ...
    public function executeTask($task_id) {
        ...
        try {
            ...
            if (!empty($task['site_id'])) {
                geoflow_set_runtime_site_id((int) $task['site_id']);
            }
    ...
    private function getTaskInfo($task_id) {
        ...
        if ($task) {
            ...
            if (geoflow_table_has_site_column($this->db, 'tasks')) {
                if ($taskSiteId <= 0) {
                    throw new Exception('任务未绑定站点');
                }

                $site = geoflow_find_site_by_id($this->db, $taskSiteId);
                if (!$site || ($site['status'] ?? 'active') !== 'active') {
                    throw new Exception('任务所属站点不存在或已停用');
                }
            }

            if ($taskSiteId > 0) {
                $siteReferences = [
                    'title_libraries' => (int) ($task['title_library_id'] ?? 0),
                    'image_libraries' => (int) ($task['image_library_id'] ?? 0),
                    'knowledge_bases' => (int) ($task['knowledge_base_id'] ?? 0),
                    'categories' => (int) ($task['fixed_category_id'] ?? 0),
                    'authors' => (int) ($task['author_id'] ?? 0),
                ];

                foreach ($siteReferences as $table => $referenceId) {
                    if ($referenceId > 0 && !geoflow_record_belongs_to_site($this->db, $table, $referenceId, $taskSiteId)) {
                        throw new Exception('任务引用了其他站点的资源：' . $table . '#' . $referenceId);
                    }
                }
            }
    ...
    private function checkDraftLimit($task_id, $draft_limit) {
        $stmt = $this->db->prepare("
            ...
            FROM articles 
            WHERE task_id = ? AND status = 'draft' AND deleted_at IS NULL" . geoflow_site_scope_sql('articles') . "
        ");
        $stmt->execute([$task_id]);
    ...
    private function saveArticle($task, $title_info, $article_data) {
        ...
        // 插入文章
        $articleHasSiteId = geoflow_table_has_site_column($this->db, 'articles');
        $siteId = (int) ($task['site_id'] ?? geoflow_current_site_id());
        $stmt = $this->db->prepare("
            INSERT INTO articles (
                " . ($articleHasSiteId ? "site_id, " : "") . "title, slug, excerpt, content, category_id, author_id, task_id,
                original_keyword, keywords, meta_description, status, review_status,
                is_ai_generated, published_at
            ) VALUES (" . ($articleHasSiteId ? "?, " : "") . "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
        ");
        ...
        if ($articleHasSiteId) {
            array_unshift($articleValues, $siteId);
        }

        $stmt->execute($articleValues);
    ...
    private function selectCategory($task) {
        $siteId = (int) ($task['site_id'] ?? geoflow_current_site_id());
        $siteScopeSql = geoflow_site_scope_condition_for_site($this->db, 'categories', $siteId);
        $siteScopeSql = $siteScopeSql !== '' ? ' AND ' . $siteScopeSql : '';

        if (($task['category_mode'] ?? 'smart') === 'fixed' && !empty($task['fixed_category_id'])) {
            $stmt = $this->db->prepare("SELECT id FROM categories WHERE id = ?" . $siteScopeSql . " LIMIT 1");
        ...
        if (($task['category_mode'] ?? '') === 'random') {
            $stmt = $this->db->prepare("SELECT id FROM categories WHERE 1=1" . $siteScopeSql . " ORDER BY RANDOM() LIMIT 1");
            $stmt->execute();
            $category = $stmt->fetch();
            if ($category) {
                return (int) $category['id'];
        ...
        $stmt = $this->db->prepare("SELECT id FROM categories WHERE 1=1" . $siteScopeSql . " ORDER BY id LIMIT 1");
        $stmt->execute();
        $category = $stmt->fetch();

        if (!$category) {
            throw new Exception('当前站点没有可用分类');
        }
    ...
    private function selectAuthor($task) {
        $siteId = (int) ($task['site_id'] ?? geoflow_current_site_id());
        $siteScopeSql = geoflow_site_scope_condition_for_site($this->db, 'authors', $siteId);
        $siteScopeSql = $siteScopeSql !== '' ? ' AND ' . $siteScopeSql : '';

        if (!empty($task['author_id'])) {
            $stmt = $this->db->prepare("SELECT id FROM authors WHERE id = ?" . $siteScopeSql . " LIMIT 1");
            $stmt->execute([(int) $task['author_id']]);
            $author = $stmt->fetch();
            if ($author) {
        ...
        // 随机选择作者
        $stmt = $this->db->prepare("SELECT id FROM authors WHERE 1=1" . $siteScopeSql . " ORDER BY RANDOM() LIMIT 1");
        $stmt->execute();
        $author = $stmt->fetch();
