...
function env_bool($key, bool $default = false): bool {
    $fallback = $default ? '1' : '0';
    return filter_var(env_value($key, $fallback), FILTER_VALIDATE_BOOLEAN);
}

function env_int($key, int $default): int {
    $value = env_value($key, (string) $default);
    return is_numeric($value) ? (int) $value : $default;
}

function resolve_app_timezone(): string {
    $candidate = trim((string) env_value('APP_TIMEZONE', env_value('TZ', 'Asia/Shanghai')));
    if ($candidate === '') {
...
// 初始化管理员配置
$bootstrapAdminUsername = trim((string) env_value('INITIAL_ADMIN_USERNAME', 'admin'));
if ($bootstrapAdminUsername === '' || !preg_match('/^[A-Za-z0-9_.-]{3,50}$/', $bootstrapAdminUsername)) {
    $bootstrapAdminUsername = 'admin';
}
define('ADMIN_USERNAME', $bootstrapAdminUsername);
...
// 安全配置
define('SESSION_NAME', trim((string) env_value('SESSION_NAME', 'blog_secure_session')) ?: 'blog_secure_session');
...
define('REQUIRE_STRONG_APP_SECRET', env_bool('REQUIRE_STRONG_APP_SECRET', false));
define('ALLOW_INSECURE_AI_TLS', env_bool('ALLOW_INSECURE_AI_TLS', false));
define('MAX_LOGIN_ATTEMPTS', max(1, env_int('MAX_LOGIN_ATTEMPTS', 5)));
define('LOGIN_LOCKOUT_TIME', max(60, env_int('LOGIN_LOCKOUT_TIME', 900)));
define('SESSION_TIMEOUT', max(300, env_int('SESSION_TIMEOUT', 3600)));
define('APP_TIMEZONE', resolve_app_timezone());

function validate_app_secret_configuration(): void {
    if (!REQUIRE_STRONG_APP_SECRET) {
        return;
    }

    $secret = trim((string) APP_SECRET_KEY);
    $isPlaceholder = $secret === '' || hash_equals($secret, SECRET_KEY);
    if ($isPlaceholder || strlen($secret) < 32) {
        throw new RuntimeException('APP_SECRET_KEY 未配置为足够强的生产密钥');
    }
}

validate_app_secret_configuration();
...
function get_initial_admin_password(): string {
    return trim((string) env_value('INITIAL_ADMIN_PASSWORD', ''));
}

function generate_bootstrap_admin_password(int $bytes = 18): string {
    return rtrim(strtr(base64_encode(random_bytes(max(12, $bytes))), '+/', '-_'), '=');
}

function build_initial_admin_credentials(): array {
    ...
    if (!is_string($passwordHash) || $passwordHash === '') {
        throw new RuntimeException('初始化管理员密码哈希生成失败');
    }
...
function store_bootstrap_admin_notice(array $credentials): ?string {
    if (empty($credentials['generated']) || empty($credentials['password'])) {
        return null;
    }

    $noticePath = sys_get_temp_dir() . '/geoflow_initial_admin_credentials.txt';
    $notice = implode(PHP_EOL, [
        'GEOflow initial admin credentials',
        'Generated at: ' . date('c'),
        'Username: ' . (string) ($credentials['username'] ?? ADMIN_USERNAME),
        'Password: ' . (string) $credentials['password'],
        'Delete this file after the first successful login.',
        '',
    ]);

    if (@file_put_contents($noticePath, $notice, LOCK_EX) !== false) {
        @chmod($noticePath, 0600);
    }

    error_log('[GEOflow] A random initial admin password was generated. Read it from ' . $noticePath);
...
function apply_ai_curl_request_defaults($ch, int $timeoutSeconds = 180, int $connectTimeoutSeconds = 10): void {
    ...
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, !ALLOW_INSECURE_AI_TLS);
    curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, ALLOW_INSECURE_AI_TLS ? 0 : 2);
...
function get_setting($key, $default = '') {
    ...
    try {
        $value = geoflow_fetch_setting_value($db, $normalized_key, null);
        if ($value !== null && $value !== '') {
            return $value;
        }
...
function set_setting($key, $value) {
    ...
    try {
        return geoflow_upsert_setting_value($db, $normalized_key, (string) $value);
    } catch (Exception $e) {
        return false;
    }
}
