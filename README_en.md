...
## Origin and Downstream Development

This repository is a downstream distribution built from the upstream
[GEOFlow](https://github.com/yaojingang/GEOFlow) project idea under the
Apache-2.0 license. Public releases keep the original
`LICENSE`, `NOTICE`, and source header attributions, while downstream changes are
...
| 🌐 Multi-Site Operations | Manage multiple sites, domains, categories, assets, and tasks from one admin with site-level isolation |
...
git clone https://github.com/alex16889/geoflow-ai-content-system.git
cd GEOFlow
...
The upstream project provides companion skills that you can reference or fork:

- Upstream skill repository: [yaojingang/yao-geo-skills](https://github.com/yaojingang/yao-geo-skills)
...
# Initial admin; set a strong password explicitly in production
INITIAL_ADMIN_USERNAME=admin
INITIAL_ADMIN_PASSWORD=

# Current public repository URL; used by admin footer and welcome links
PROJECT_GITHUB_URL=https://github.com/alex16889/geoflow-ai-content-system

# Optional: DataForSEO keyword import. Keep real values in private environments only.
DATAFORSEO_LOGIN=
DATAFORSEO_PASSWORD=
```
...
Visit `/geo_admin/` and sign in with the admin account. The initial username comes from `INITIAL_ADMIN_USERNAME` and defaults to `admin`. For production, set `INITIAL_ADMIN_PASSWORD` explicitly. If it is not set, GEOFlow generates a random initial password and writes the one-time notice to `/tmp/geoflow_initial_admin_credentials.txt`. Change the password immediately after first login.
...
> ⚠️ Before production deployment, set a strong random `APP_SECRET_KEY` and manage the admin password through `INITIAL_ADMIN_PASSWORD` or the admin security settings after first login.
>
...
- [Upstream Companion Skill Repository](https://github.com/yaojingang/yao-geo-skills) - `geoflow-cli-ops`
