...
try {
    // 获取标题库信息
    $stmt = $db->prepare("SELECT * FROM title_libraries WHERE id = ?" . geoflow_site_scope_sql('title_libraries'));
    ...
    // 获取关键词库列表
    $keyword_libraries = $db->query("SELECT * FROM keyword_libraries WHERE 1=1" . geoflow_site_scope_sql('keyword_libraries') . " ORDER BY created_at DESC")->fetchAll();
    ...
    $default_keyword_library_id = (int) ($keyword_libraries[0]['id'] ?? 0);
    $default_ai_model_id = (int) ($ai_models[0]['id'] ?? 0);
    $selected_keyword_library_id = (int) ($_POST['keyword_library_id'] ?? $default_keyword_library_id);
    $selected_ai_model_id = (int) ($_POST['ai_model_id'] ?? $default_ai_model_id);
    $selected_title_count = (int) ($_POST['title_count'] ?? 10);
    $selected_title_count = max(1, min(50, $selected_title_count));
    $selected_title_style = (string) ($_POST['title_style'] ?? 'professional');
    if (!in_array($selected_title_style, ['professional', 'attractive', 'seo', 'creative', 'question'], true)) {
        $selected_title_style = 'professional';
    }
    $selected_custom_prompt = trim((string) ($_POST['custom_prompt'] ?? ''));
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'generate_titles') {
            $keyword_library_id = $selected_keyword_library_id;
            $ai_model_id = $selected_ai_model_id;
            $custom_prompt = $selected_custom_prompt;
            $title_count = $selected_title_count;
            $title_style = $selected_title_style;
            ...
            // 获取关键词库信息
            $stmt = $db->prepare("SELECT * FROM keyword_libraries WHERE id = ?" . geoflow_site_scope_sql('keyword_libraries'));
...
?>
            ...
            <form method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="keyword_library_id" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <?php foreach ($keyword_libraries as $kw_lib): ?>
                                <option value="<?php echo $kw_lib['id']; ?>" <?php echo (int) $kw_lib['id'] === $selected_keyword_library_id ? 'selected' : ''; ?>>
                    ...
                    <div>
                        ...
                        <select name="ai_model_id" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <?php foreach ($ai_models as $model): ?>
                                <option value="<?php echo $model['id']; ?>" <?php echo (int) $model['id'] === $selected_ai_model_id ? 'selected' : ''; ?>>
                    ...
                    <div>
                        ...
                        <input type="number" name="title_count" value="<?php echo $selected_title_count; ?>" min="1" max="50" 
                    ...
                    <div>
                        ...
                        <select name="title_style" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            <option value="professional" <?php echo $selected_title_style === 'professional' ? 'selected' : ''; ?>><?php echo __('title_ai_generate.style.professional'); ?></option>
                            <option value="attractive" <?php echo $selected_title_style === 'attractive' ? 'selected' : ''; ?>><?php echo __('title_ai_generate.style.attractive'); ?></option>
                            <option value="seo" <?php echo $selected_title_style === 'seo' ? 'selected' : ''; ?>><?php echo __('title_ai_generate.style.seo'); ?></option>
                            <option value="creative" <?php echo $selected_title_style === 'creative' ? 'selected' : ''; ?>><?php echo __('title_ai_generate.style.creative'); ?></option>
                            <option value="question" <?php echo $selected_title_style === 'question' ? 'selected' : ''; ?>><?php echo __('title_ai_generate.style.question'); ?></option>
                        </select>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="custom_prompt" rows="4" 
                              class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                              placeholder="<?php echo __('title_ai_generate.placeholder.custom_prompt'); ?>"><?php echo htmlspecialchars($selected_custom_prompt); ?></textarea>
    ...
    <script>
        (function() {
        const initTitleAIGeneratePage = function() {
            ...
            const keywordLibrarySelect = form ? form.querySelector('select[name="keyword_library_id"]') : null;
            const aiModelSelect = form ? form.querySelector('select[name="ai_model_id"]') : null;

            let progressInterval = null;
            let startTime = null;
            let currentTaskId = null;
            const buildAdminUrl = function(path) {
            ...
            function startAsyncGenerate() {
                ...
                if (!keywordLibrarySelect || !keywordLibrarySelect.value) {
                    if (keywordLibrarySelect) {
                        keywordLibrarySelect.focus();
                    }
                    alert('<?php echo addslashes(__('title_ai_generate.error.keyword_library_required')); ?>');
                    return;
                }
                ...
                if (!aiModelSelect || !aiModelSelect.value) {
                    ...
                    alert('<?php echo addslashes(__('title_ai_generate.error.ai_model_required')); ?>');
                    return;
                }

                // 获取表单数据
                const formData = new FormData(form);
                formData.set('action', 'start_generate');
                formData.set('library_id', '<?php echo $library_id; ?>');
                ...
                fetch(buildAdminUrl('title_generate_async.php'), {
                    method: 'POST',
                    body: formData
                })
            ...
            function updateProgress() {
                ...
                fetch(`${buildAdminUrl('title_generate_async.php')}?action=get_progress&task_id=${encodeURIComponent(currentTaskId)}`)
                .then(response => response.json())
                .then(data => {
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initTitleAIGeneratePage, { once: true });
        } else {
            initTitleAIGeneratePage();
        }
        })();
    </script>
