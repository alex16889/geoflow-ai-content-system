...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $loginThrottleKey = 'web:' . (get_admin_request_ip() ?: 'unknown');

    // 验证CSRF令牌
    if (!verify_csrf_token($csrf_token)) {
        $error = __('login.error.invalid_request');
    } elseif (!check_login_attempts($loginThrottleKey)) {
        ...
        if ($admin && ($admin['status'] ?? 'active') === 'active' && password_verify($password, $admin['password'])) {
            record_login_attempt($loginThrottleKey, true);
            ...
            record_login_attempt($loginThrottleKey, false);
...
<body>
    <div class="fixed right-4 top-4 z-50">
        <select data-language-switch class="rounded-md border border-gray-300 bg-white px-2 py-1 text-xs text-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
