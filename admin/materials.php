...
session_write_close();
$keywordLibrariesSiteCondition = geoflow_site_scope_condition('keyword_libraries', 'kl');
$titleLibrariesSiteCondition = geoflow_site_scope_condition('title_libraries', 'tl');
$imageLibrariesSiteCondition = geoflow_site_scope_condition('image_libraries', 'il');
$knowledgeBasesSiteCondition = geoflow_site_scope_condition('knowledge_bases');
$authorsSiteCondition = geoflow_site_scope_condition('authors');

// 获取统计数据
$stats = [
    'keyword_libraries' => $db->query("SELECT COUNT(*) as count FROM keyword_libraries WHERE 1=1" . geoflow_site_scope_sql('keyword_libraries'))->fetch()['count'] ?? 0,
    'total_keywords' => $db->query("SELECT COUNT(*) as total FROM keywords k INNER JOIN keyword_libraries kl ON kl.id = k.library_id" . ($keywordLibrariesSiteCondition !== '' ? ' WHERE ' . $keywordLibrariesSiteCondition : ''))->fetch()['total'] ?? 0,
    'title_libraries' => $db->query("SELECT COUNT(*) as count FROM title_libraries WHERE 1=1" . geoflow_site_scope_sql('title_libraries'))->fetch()['count'] ?? 0,
    'total_titles' => $db->query("SELECT COUNT(*) as total FROM titles t INNER JOIN title_libraries tl ON tl.id = t.library_id" . ($titleLibrariesSiteCondition !== '' ? ' WHERE ' . $titleLibrariesSiteCondition : ''))->fetch()['total'] ?? 0,
    'image_libraries' => $db->query("SELECT COUNT(*) as count FROM image_libraries WHERE 1=1" . geoflow_site_scope_sql('image_libraries'))->fetch()['count'] ?? 0,
    'total_images' => $db->query("SELECT COUNT(*) as total FROM images i INNER JOIN image_libraries il ON il.id = i.library_id" . ($imageLibrariesSiteCondition !== '' ? ' WHERE ' . $imageLibrariesSiteCondition : ''))->fetch()['total'] ?? 0,
    'knowledge_bases' => $db->query("SELECT COUNT(*) as count FROM knowledge_bases WHERE 1=1" . geoflow_site_scope_sql('knowledge_bases'))->fetch()['count'] ?? 0,
    'authors' => $db->query("SELECT COUNT(*) as count FROM authors WHERE 1=1" . geoflow_site_scope_sql('authors'))->fetch()['count'] ?? 0
];
...
<div class="flex items-center justify-between">
    ...
    <div class="flex space-x-3">
        <a href="seo-material-wizard.php" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
...
?>
        ...
        <div class="mb-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_360px]">
                <div class="bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 px-8 py-8 text-white">
                    <div class="inline-flex items-center rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-xs font-semibold text-cyan-100">
                        <i data-lucide="sparkles" class="mr-2 h-3.5 w-3.5"></i>
                        给不会 SEO 的人用
                    </div>
                    <h2 class="mt-5 text-3xl font-black tracking-tight">一键把标题、图片、AI知识库都搭好</h2>
                    <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-200">
                        先根据当前站点资料、关键词库、分类和已有文章生成完整 SEO 素材包。你后面创建任务时直接选择这些库，不需要手动想标题、写图片 alt 或整理知识库。
                    </p>
                    <div class="mt-6 flex flex-wrap gap-3">
                        <a href="seo-material-wizard.php" class="inline-flex items-center rounded-2xl bg-white px-5 py-3 text-sm font-bold text-slate-950 shadow-lg hover:bg-slate-100">
                            <i data-lucide="wand-sparkles" class="mr-2 h-5 w-5"></i>
                            进入 SEO素材助手
                        </a>
                        <span class="inline-flex items-center rounded-2xl border border-white/15 px-5 py-3 text-sm font-semibold text-slate-200">
                            不消耗 DataForSEO 拉词额度
                        </span>
                    </div>
                </div>
                <div class="border-t border-slate-200 bg-slate-50 p-8 lg:border-l lg:border-t-0">
                    <h3 class="text-base font-bold text-slate-950">生成内容</h3>
                    <div class="mt-5 space-y-4 text-sm text-slate-600">
                        <div class="flex items-start gap-3">
                            <i data-lucide="type" class="mt-0.5 h-5 w-5 text-blue-600"></i>
                            <div><span class="font-semibold text-slate-900">标题库</span>：长尾词、FAQ、入口说明、问题排查类标题。</div>
                        </div>
                        <div class="flex items-start gap-3">
                            <i data-lucide="image" class="mt-0.5 h-5 w-5 text-violet-600"></i>
                            <div><span class="font-semibold text-slate-900">图片库</span>：信息图卡片、alt、caption 和图片提示词。</div>
                        </div>
                        <div class="flex items-start gap-3">
                            <i data-lucide="brain" class="mt-0.5 h-5 w-5 text-orange-600"></i>
                            <div><span class="font-semibold text-slate-900">AI知识库</span>：站点定位、内容模板、风险边界、内链规则。</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white shadow rounded-lg">
                ...
                <div class="px-6 py-6">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        ...
                        <a href="keyword-research.php" class="inline-flex items-center px-4 py-2 border border-emerald-200 text-sm font-medium rounded-md text-emerald-700 bg-emerald-50 hover:bg-emerald-100">
                            <i data-lucide="sparkles" class="w-4 h-4 mr-2"></i>
                            DataForSEO 拉词
                        </a>
                    </div>
        ...
        <div class="bg-white shadow rounded-lg">
            ...
            <div class="px-6 py-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    <a href="seo-material-wizard.php" class="flex items-center p-4 border border-blue-200 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
                        <i data-lucide="sparkles" class="w-8 h-8 text-blue-600 mr-3"></i>
                        <div>
                            <h4 class="font-medium text-gray-900">SEO素材助手</h4>
                            <p class="text-sm text-gray-500">一键生成素材</p>
                        </div>
                    </a>
