...
| 🌐 多站点精品站管理 | 单后台管理多个站点、域名、分类、素材和任务，核心数据按站隔离 |
...
上游项目配套提供了两个公开 skill，可按需参考或另行维护你自己的 fork：

- 上游 Skill 仓库：[yaojingang/yao-geo-skills](https://github.com/yaojingang/yao-geo-skills)
...
1. 登录后台  
访问 `/geo_admin/`，使用管理员账号进入后台。默认用户名来自 `INITIAL_ADMIN_USERNAME`，默认是 `admin`。生产环境建议显式设置 `INITIAL_ADMIN_PASSWORD`；如果未设置，系统会生成随机初始密码，并把一次性提示写入运行时临时目录 `/tmp/geoflow_initial_admin_credentials.txt`。首次登录后请立即修改密码。
...
> ⚠️ 生产部署前请务必设置强随机 `APP_SECRET_KEY`，并通过 `INITIAL_ADMIN_PASSWORD` 或首次登录后的后台设置管理管理员密码。
>
...
- [上游配套 Skill 仓库](https://github.com/yaojingang/yao-geo-skills) - `geoflow-cli-ops`
