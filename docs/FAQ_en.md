...
- Default admin password: there is no fixed default password. Set `INITIAL_ADMIN_PASSWORD` for production; otherwise GEOFlow generates a random initial password and writes it to `/tmp/geoflow_initial_admin_credentials.txt`.
